...
st.markdown("""
    <style>
        ...
        /* === 2. INPUTS DE TEXTO (FONDO BLANCO) === */
        ...
        /* Botones +/- numéricos y Flechas Carrusel */
        /* Usamos un selector específico para los botones pequeños */
...
def render_carousel(label, options, key_name):
    """Crea un selector tipo carrusel manual"""
    ...
    with c1:
        if st.button("◀", key=f"prev_{key_name}"):
            # Restar 1, con vuelta al final si es < 0
            new_idx = (st.session_state[f"{key_name}_idx"] - 1) % len(options)
            st.session_state[f"{key_name}_idx"] = new_idx
            st.rerun()
            
    with c3:
        if st.button("▶", key=f"next_{key_name}"):
            # Sumar 1, con vuelta al inicio si pasa el largo
            new_idx = (st.session_state[f"{key_name}_idx"] + 1) % len(options)
            st.session_state[f"{key_name}_idx"] = new_idx
            st.rerun()
...
# --- 5. TRADUCCIONES (CORREGIDAS SIN ERRORES DE SINTAXIS) ---
t = {
    'fr': {
        ...
        'disclaimer_text': "Projet indépendant. PAS avocats/consultants.",
        ...
        'arrival_label': "Date d'arrivée (Ex: 2025-07-01)",
        ...
        'sp_fr_title': "Français Conjoint", 'sp_fr_label': "Niveau Oral",
...
with main_tab_calc:
    ...
    if st.session_state.step == 1:
        ...
        if "bec" not in st.session_state.current_loc:
             ...
             st.session_state.arrival_text = st.text_input("DateInput", placeholder="YYYY-MM-DD", label_visibility="collapsed")
        ...
        if st.session_state.spouse:
            ...
            with c_sp2: 
                ...
                st.session_state.sp_edu = st.radio("SpEdu", ["PhD", "Master", "Bachelor", "Technical", "Secondary"], index=2, label_visibility="collapsed")
